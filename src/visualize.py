...
def draw_baseline():
    ...
    for i in range(len(freqs)):
        plt.axline((0, freqs[i]), (1, freqs[i]), linewidth=1, linestyle = '-' if (i % 12) in major else '--')
...
def pitch_visualize(pitch, audio_file):
    ...
    # add the input soundtrack to 
    moving_axis.save('temp.mp4', writer='ffmpeg')
    input_video = 'temp.mp4'
    output_video = 'output.mp4'

    command = ["ffmpeg", "-i", input_video, "-i", audio_file, "-c:v", "copy", "-c:a", "aac", "-strict", "experimental", output_video]
